...
- **Worker deploy on FOC shared setup**: `nova worker deploy --name foc-gateway-proxy --kv foc-site-cids --r2 foc-site-cache`. Nova defaults (`filecoin-nova-gateway`, `filecoin-nova-gateway-kv`) don't match the pre-cutover Worker. Using defaults rebinds the script to an empty KV namespace and 404s every site with "No CID registered".
- **Worker content cache (v0.7.12)**: site content is cached by `<cid><pathname>`, not by hostname URL, in the R2 bucket bound as `SITE_CACHE` (durable, shared by every colo) and in the per-colo Cache API with a one-year TTL. A KV flip on deploy changes the key, so no cache purge is needed. Public IPFS gateways (dweb.link, w3s.link, ipfs.io) are only hit once per asset per deploy; they rate-limit backend traffic with 429s since Aug 2026 (IPFS Foundation policy), so never rely on them per request again. Script uploads send `keep_bindings: ["secret_text"]` (and `r2_bucket` when no `--r2` is given) so the FilBeam D1 secrets on the live Worker survive a re-upload.
- **Worker source and build (v0.7.13)**: the Worker lives in `src/worker/index.js` + `src/worker/foc.js` and is bundled by esbuild into `dist/worker.js` (`pnpm build`); `src/worker.js` no longer exists. `foc.js` retrieves content straight from FOC storage providers: providers for the CID come from `delegated-ipfs.dev/routing/v1/providers` (cached 10 min in the Cache API), the path is fetched as a CAR with `dag-scope=entity` from the provider's trustless gateway, every block is sha2-256 verified, and `ipfs-unixfs-exporter` yields the file. Enabled for every host (`FOC_DIRECT_HOSTS = null`; set it to a `Set` of hostnames to stage a change); the Cache API key origin carries a generation (`foc-site-cache-v2.internal`), bump it to force every colo to refill from R2; on those hosts R2 entries whose `customMetadata.source` is not `foc:*` are refilled from FOC (public gateways inject a hidden Cloudflare honeypot `<a>` into HTML, so gateway-sourced copies are not byte-exact). Public gateways are the last resort only.
- **Worker-only changes** (gateway fallback chain, KV-writer logic): use `nova worker deploy`, not a full CI site dispatch. Full site redeploy also re-uploads the Worker but adds FOC pin + DNSLink + ENS tx per site (wasted gas on idempotent runs).
